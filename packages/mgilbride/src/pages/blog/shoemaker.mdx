...
count me amongst the many that think its reign as the de-facto way to build web apps won't last forever.
...
Python, however, doesn't run in a browser (yet).  So for now, those of us targeting the web ends have to
make other choices.
...
We live in a crazy world, so we might as well write Javascript that emits CSS to because, well, who
...
(as opposed to string interpolations).  This allows me to statically check my styles.  I can leverage the great CSSType
