...
A couple of weeks ago, I sat down with Claude Code — Anthropic's CLI tool — to tackle some long-overdue technical debt on a couple of side projects. I wanted to migrate off Prismic CMS, modernize some styling, fix some mobile layout bugs, and clean up years of accumulated cruft.
...
**Debugging required partnership, not delegation.** A mobile layout overflow bug took multiple rounds to resolve. Claude did the actual investigating — inspecting computed styles through a browser extension, measuring element widths via JavaScript — but it kept addressing symptoms rather than root causes. It needed me to step in and say "stop patching, the grid itself is the problem." The fix was simple once we got there. But getting there required someone who could recognize when the approach was wrong, not just when the output was wrong.
